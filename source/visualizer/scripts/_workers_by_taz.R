workers_by_taz <- function(land_use_dir, asim_config_dir, zones_dir, wd, alternative_sample_rate,
                           hh, per) {
# read data
setwd(land_use_dir)
...
per$finalweight <- 1/as.numeric(alternative_sample_rate)
per$income_segment <- hh$income_segment[match(per$household_id, hh$household_id)]

workers <- per[per$workplace_zone_id > 0,]
...
workersbyTAZ <- workers %>%
  ...
  select(workplace_zone_id, COUNTY, tot_workers, tot_emp)
